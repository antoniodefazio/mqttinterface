...
@Slf4j
@AllArgsConstructor
public class ItaStringToMqttMessage implements Converter<String,MqttObuMessageInt>{
	...
	public MqttObuMessageInt convert(String json) {
		...
		try {
			return objectMapper.readValue(json, MqttObuMessageIta.class);
		} catch (JsonProcessingException e) {
			log.error("Malformed MqttObuMessageIta JSON: {}", json);
